...
Generate AI-designed thumbnails from a source video. **7 credits per call.** Pro + Enterprise only. Rides the shared `/generative-jobs` endpoint — there is no dedicated `/thumbnails` path.
